...
@section('content')
    ...
    @can('managePermissions', $role)
        <form method="POST" action="{{ route('role-permissions.toggle', [$role, $permission]) }}" class="inline">
            @csrf
            @method('PUT')
        @endcan
        ...
        @forelse ($permissions as $permission)
            ...
            <button type="submit" class="btn-sm {{ $role->hasPermissionTo($permission) ? 'btn-danger' : 'btn-success' }}">
